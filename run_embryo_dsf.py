...
import numpy as np
import math
import SF_nd    #step function parameterization for volatile frac
...
def calc(input_index,input_div):
    ...
    for filename in sorted(glob.glob('nbody_sims/*to*')):
    	...
    	with open( filename + "/finalplanets.txt", 'r') as f1:
    	...
    	with open( filename + "/aorig.dat", 'r') as aorig:
    	...
    	with open(filename + "/planetgrowth.out",'r') as f:
    		...
    		for line in lines_pl:  #read in entire file and separate by column
    			...
    			if initial:  #initialize values of each gas speices
    				if target_i == target_i_file:
    					...
    					water_m= target_m*SF_nd.water_mf(target_au)
    					carbon_m=  target_m*SF_nd.carbon_mf(target_au)
    					nitrogen_m= target_m*SF_nd.nitrogen_mf(target_au)
    			...
    			if target_i ==  target_i_file:  #change target index to desired embryo
    				...
    				if int(project_i) <= emb_divide: # for embryos
    					...
    					water_a= water_a + project_m*SF_nd.water_mf(project_au)*0.01
    					carbon_a= carbon_a + project_m*SF_nd.carbon_mf(project_au)*0.01
    					nitrogen_a= nitrogen_a+ project_m*SF_nd.nitrogen_mf(project_au)*0.01
    					accret_tot = project_m*SF_nd.water_mf(project_au) + project_m*SF_nd.carbon_mf(project_au)+ project_m*SF_nd.nitrogen_mf(project_au)
					...
					#############################Henry's Law###################
    					...
    					while Mcurr <= project_m:
    						...
    						water_a= water_a + Mpl*SF_nd.water_mf(project_au)
    						carbon_a= carbon_a + Mpl*SF_nd.carbon_mf(project_au)
    						nitrogen_a= nitrogen_a + Mpl*SF_nd.nitrogen_mf(project_au)
    						
    						accret_tot = Mpl*SF_nd.water_mf(project_au) + Mpl*SF_nd.carbon_mf(project_au) +  Mpl*SF_nd.nitrogen_mf(project_au)
