...
ab_i = []
ab_radii = []
with open( "ABsizes_bottke.dat", 'r') as f:
...
counter =0
bottke_radii = np.linspace(600,1500,15)
...
with open(filepath + "/planetgrowth.out",'r') as f:
        ...
        for line in lines_pl:  #read in entire file and separate by column
                ...
                if initial:  #initialize values of each gas speices
                        if target_i == target_i_file:
                                ...
                                water_a=1e23# target_m*SF.water_mf(target_au)*0.005
        ...
        #########################hydrodynamics escape, energy-limited##############
                                ...
                                water_m_eq= (henry.kH_t(Ta,'water')*Psurf_bar*water_molfrac)*water_mm*target_m*0.68
                                carbon_m_eq  = (henry.kH_t(Ta,'carbon')*Psurf_bar*carbon_molfrac)*carbon_mm*target_m*0.68
                                nitrogen_m_eq = (henry.kH_t(Ta,'nitrogen')*Psurf_bar*nitrogen_molfrac)*nitrogen_mm*target_m*0.68
                                ...
                                fracEarth2_List.append(round(target_m/Mearth,3))
                                ...
                                ingassed_List.append('{:0.3e}'.format(ingass_tot))
                                degassed_List.append('{:0.3e}'.format(degass_tot))
                                nitrogenm_List.append('{:0.3e}'.format(nitrogen_m))
                                carbon_add_List.append('{:0.3e}'.format(emb_carbon- impactEsc*carbon_mol_a))
                                nitrogen_add_List.append('{:0.3e}'.format(emb_nitrogen-impactEsc*nitrogen_mol_a))
                                carbon_degass_List.append('{:0.3e}'.format(degass_carbon-ingass_carbon))
                                nitrogen_degass_List.append('{:0.3e}'.format(degass_nitrogen - ingass_nitrogen))
               ...
               #########################hydrodynamics escape, energy-limited##############
                                        ...
                                        water_m_eq= (henry.kH_tf(Ta,'water')*Psurf_bar*water_molfrac)*water_mm*target_m*0.68  #molarity * molar mass = grams/litter, (molarity * mass)* solution mass = grams
                                        carbon_m_eq  = (henry.kH_tf(Ta,'carbon')*Psurf_bar*carbon_molfrac)*carbon_mm*target_m*0.68
                                        nitrogen_m_eq = (henry.kH_tf(Ta,'nitrogen')*Psurf_bar*nitrogen_molfrac)*nitrogen_mm*target_m*0.68
                                        ...
                                        if (Rpl > 1e8 or random.uniform(0,1) < 1.8e-4):
                                                ...
                                                accret_List.append('{:0.3e}'.format(accret_tot))	
                                                carbon_add_List.append('{:0.3e}'.format(Mpl*SF.carbon_mf(project_au)- impactEsc*carbon_mol_a))
                                                nitrogen_add_List.append('{:0.3e}'.format(Mpl*SF.nitrogen_mf(project_au) - impactEsc*nitrogen_mol_a))
                                                carbon_degass_List.append('{:0.3e}'.format(degass_carbon-ingass_carbon))
                                                nitrogen_degass_List.append('{:0.3e}'.format(degass_nitrogen - ingass_nitrogen))
...
with open('reference/OUT_ATM_ref', 'w') as f2:
...
with open('reference/OUT_MANT_ref', 'w') as f4:
...
with open('reference/OUT_FLUX_ref', 'w') as f6:
		...
		f6.write("%s %s %s %s %s %s %s %s %s %s" % ("Earth", "Esc", "Accreted", "Ingassed", "Degassed", "addCarbon", "addNitrogen", "deCarbon", "deNitrogen", "radius of impactor \n"))
		writer.writerows(zip(fracEarth2_List, esctot_List,  accret_List, ingassed_List, degassed_List, carbon_add_List, nitrogen_add_List, carbon_degass_List, nitrogen_degass_List, rpl_List))
