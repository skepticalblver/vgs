...
import math
import SF_flat
import SF_flat_rp  #step function parameterization for volatile frac
			...
			if target_i ==  target_i_file:  #change target index to desired embryo
                ...
                #########################hydrodynamics escape, energy-limited##############
					...
					while Mcurr <= project_m:
						...
						water_a= water_a + Mpl*float(SF_flat_rp.water_mf(project_au))
						carbon_a= carbon_a + Mpl*float(SF_flat_rp.carbon_mf(project_au))
						nitrogen_a= nitrogen_a + Mpl*float(SF_flat_rp.nitrogen_mf(project_au))
						
						accret_tot = Mpl*float(SF_flat_rp.water_mf(project_au)) + Mpl*float(SF_flat_rp.carbon_mf(project_au)) +  Mpl*float(SF_flat_rp.nitrogen_mf(project_au))
...
with open('comp/OUT_ATM_flatcomp', 'w') as f2:
...
with open('comp/OUT_MANT_flatcomp', 'w') as f4:
