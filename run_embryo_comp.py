    		...
    		for line in lines_pl:  #read in entire file and separate by column
    			...
    			if initial:  #initialize values of each gas speices
    				if target_i == target_i_file:
    					...
    					water_m= target_m*float(SF_variable_rp.water_mf(target_au))
    					carbon_m=  target_m*float(SF_variable_rp.carbon_mf(target_au))
    					nitrogen_m= target_m*float(SF_variable_rp.nitrogen_mf(target_au))
    			...
    			if target_i ==  target_i_file:  #change target index to desired embryo
    				...
    				if int(project_i) <= emb_divide: # for embryos
    					...
    					water_a= water_a + project_m*float(SF_variable_rp.water_mf(project_au))*0.01
    					carbon_a= carbon_a + project_m*float(SF_variable_rp.carbon_mf(project_au))*0.01
    					nitrogen_a= nitrogen_a+ project_m*float(SF_variable_rp.nitrogen_mf(project_au))*0.01
    					accret_tot = project_m*float(SF_variable_rp.water_mf(project_au)) + project_m*float(SF_variable_rp.carbon_mf(project_au))+ project_m*float(SF_variable_rp.nitrogen_mf(project_au))
